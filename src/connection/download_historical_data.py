...
def _download_historical_data_from_binance(time_window: TimeWindow, security: TradingPair, api_interval_callback: str,
...
def _download_historical_data_from_cobinhood(time_window: TimeWindow, security: TradingPair,
...
def _download_historical_data_from_exchange(time_window: TimeWindow, security: TradingPair,
                                            api_interval_callback: timedelta,
                                            client: Union[BinanceClient, CobinhoodClient]) -> List[Candle]:
    if isinstance(client, BinanceClient):

        return _download_historical_data_from_binance(time_window, security,
                                                      binance_sampling_rate_mappings[api_interval_callback.seconds],
                                                      client)
    elif isinstance(client, CobinhoodClient):
        return _download_historical_data_from_cobinhood(time_window, security,
