...
def _download_live_data_from_binance(security: TradingPair, api_interval_callback: timedelta,
                                     client: BinanceClient) -> List[Candle]:
    klines = client.get_historical_klines(security.as_string_for_binance(),
...
def _download_live_data_from_cobinhood(security: TradingPair, api_interval_callback: timedelta,
...
def _download_live_data_from_exchange(security: TradingPair, api_interval_callback: timedelta,
                                      client: Union[BinanceClient, CobinhoodClient]) -> List[Candle]:
    if isinstance(client, BinanceClient):

        return _download_live_data_from_binance(security, api_interval_callback, client)
    elif isinstance(client, CobinhoodClient):
        return _download_live_data_from_cobinhood(security, api_interval_callback, client)
    else:
        raise DownloadingError("Invalid client object.")


@retry_on_network_error
def download_live_data(client: Union[BinanceClient, CobinhoodClient], security: TradingPair,
                       api_interval_callback: timedelta, lags: int) -> Candle:
    candle = _download_live_data_from_exchange(security, api_interval_callback, client)[-1]
    print(candle)
